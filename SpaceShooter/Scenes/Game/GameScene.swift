...
class GameScene: SKScene {
    ...
    @objc func addAliensAndAsteroids() {
        ...
        attaker.position = CGPoint(x: position, y: frame.size.height)
    ...
    func fireTorpedo() {
        ...
        torpedoNode.size = CGSize(width: 100, height: 100)
        torpedoNode.physicsBody = SKPhysicsBody(circleOfRadius: torpedoNode.size.width/10)
...
extension GameScene: SKPhysicsContactDelegate {
    ...
    override func didSimulatePhysics() {
        player.position.x += xAcceleration * 15
        if player.position.x < -frame.size.width/10 {
        ...
        } else if player.position.x == starField.position.x/2 {
            player.position.x += xAcceleration * 0
        }
    }
}
